				...
				<div class="col-md-3">
					<div class="form-group{{ $errors->has('floor_from') ? ' has-error' : '' }}">
					    ...
					    {!! Form::text('floor_from', null, ['class' => 'form-control', 'required' => 'required']) !!}
				...
				<div class="col-md-3">
					<div class="form-group{{ $errors->has('floor_to') ? ' has-error' : '' }}">
					    ...
					    {!! Form::text('floor_to', null, ['class' => 'form-control', 'required' => 'required']) !!}
					    <small class="text-danger">{{ $errors->first('floor_to') }}</small>
					</div>
				</div>
				
				<div class="col-md-12">
					<div class="form-group{{ $errors->has('room_numbers') ? ' has-error' : '' }}">
					    {!! Form::label('room_numbers', 'عدد الغرف') !!}
					    {!! Form::text('room_numbers', null, ['class' => 'form-control', 'required' => 'required']) !!}
					    <small class="text-danger">{{ $errors->first('room_numbers') }}</small>
					</div>
				</div>
							...
							<div class="form-group">
							    <div class="checkbox{{ $errors->has('worker_'.$type->id) ? ' has-error' : '' }}">
							        <label for="worker_{{$type->id}}">
							            {!! Form::checkbox('workers[]', $type->id, null, ['class'=>'workers','id' => 'worker_'.$type->id]) !!}{{$type->name}}
							        </label>
							        {!!Form::text('workers_number['.$type->id.']',null,['style'=>'display:none','class'=>'form-control','placeholder'=>'العدد'])!!}
							    </div>
							    <small class="text-danger">{{ $errors->first('is_throupass') }}</small>
							</div>
						@endforeach
				...
				$(document).ready(function(){
					...
					$('.workers').each(function () {
					    if($(this).checked){
							$(this).parent().parent().find('input[type="text"]').show('slow');
					    }else{
							$(this).parent().parent().find('input[type="text"]').hide('slow');
					    } 
					});
